    ...
    try
    {
        ...
        case 1:
            deal_with_duplicates(
                action, find_duplicates<uint8_t>(cl_args));
            break;
        case 2:
            deal_with_duplicates(
                action, find_duplicates<uint16_t>(cl_args));
            break;
        case 4:
            deal_with_duplicates(
                action, find_duplicates<uint32_t>(cl_args));
            break;
        default:
            deal_with_duplicates(
                action, find_duplicates<uint64_t>(cl_args));
            break;
        }
    }
